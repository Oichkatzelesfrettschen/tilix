...
let signed_int_eq_dec x y =
  let b = signed_int_beq x y in if b then true else false
...
let hexadecimal_beq x y =
  ...
  | Hexadecimal (i, f) ->
    (match y with
     | Hexadecimal (i0, f0) -> (&&) (signed_int_beq i i0) (uint_beq f f0)
  ...
  | HexadecimalExp (i, f, e) ->
    (match y with
     ...
     | HexadecimalExp (i0, f0, e0) ->
       (&&) (signed_int_beq i i0)
         ((&&) (uint_beq f f0) (Decimal.signed_int_beq e e0)))
...
(** val nb_digits : uint -> int **)

let rec nb_digits = function
| Nil -> 0
| D0 d0 -> Stdlib.Int.succ (nb_digits d0)
| D1 d0 -> Stdlib.Int.succ (nb_digits d0)
| D2 d0 -> Stdlib.Int.succ (nb_digits d0)
| D3 d0 -> Stdlib.Int.succ (nb_digits d0)
| D4 d0 -> Stdlib.Int.succ (nb_digits d0)
| D5 d0 -> Stdlib.Int.succ (nb_digits d0)
| D6 d0 -> Stdlib.Int.succ (nb_digits d0)
| D7 d0 -> Stdlib.Int.succ (nb_digits d0)
| D8 d0 -> Stdlib.Int.succ (nb_digits d0)
| D9 d0 -> Stdlib.Int.succ (nb_digits d0)
| Da d0 -> Stdlib.Int.succ (nb_digits d0)
| Db d0 -> Stdlib.Int.succ (nb_digits d0)
| Dc d0 -> Stdlib.Int.succ (nb_digits d0)
| Dd d0 -> Stdlib.Int.succ (nb_digits d0)
| De d0 -> Stdlib.Int.succ (nb_digits d0)
| Df d0 -> Stdlib.Int.succ (nb_digits d0)
...
(** val del_head_int : int -> signed_int -> uint **)

let del_head_int n = function
| Pos d0 -> del_head n d0
| Neg d0 -> del_head n d0

(** val del_tail : int -> uint -> uint **)

let del_tail n d =
  rev (del_head n (rev d))

(** val del_tail_int : int -> signed_int -> signed_int **)
